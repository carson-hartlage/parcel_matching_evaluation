...
```{r}
d <- readRDS("~/Documents/GitHub/parcel_matching_evaluation/NAD_matching_data_10.21.25.rds")
...
agreement_equal_parcel <- function(col1, col2) {
  mapply(function(id, id_list) {
    if (is.na(id) && (is.null(id_list) || all(is.na(id_list)))) return(TRUE)
    
    if (!is.na(id) && !is.null(id_list)) {
      vals <- unlist(id_list)
      vals <- vals[!is.na(vals)]
      if (length(vals) == 0) return(FALSE)
      return(as.character(id) %in% as.character(vals))
    }
...
```{r}
rates <- data.frame(
  county = c(rep("Hamilton County", 3), rep("Franklin County", 3)),
  method = rep(c("address matching", "geomatching (parcel-based)", "geomatching (range-based)"), 2),
...
```{r}
rates <- data.frame(
  method = c("addr", "NAD_geo", "DeGAUSS_geo"),
...
## Tract-level deprivation index and address density
```{r}
ham_by_tract <- ham |>
