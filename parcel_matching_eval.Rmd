...
rates <- data.frame(
  ...
  method = rep(c("address tag matching", "parcel-based geomatching to centroid", "range-based geomatching to centroid", "parcel-based geomatching to polygon", "range-based geomatching to polygon"), 2),
...
```{r}
rates <- data.frame(
  method = c("address tag matching", "parcel-based geomatching to centroid", "range-based geomatching to centroid", "parcel-based geomatching to polygon", "range-based geomatching to polygon"),
...
```{r}
hcv_frank <- readRDS("/Users/carsonhartlage/Documents/GitHub/parcel_matching_evaluation/franklin_hcv_by_address.rds")
hcv_ham <- readRDS("/Users/carsonhartlage/Documents/GitHub/parcel_matching_evaluation/hamilton_hcv_by_address.rds")

#hcv <- vctrs::vec_rbind(hcv_frank, hcv_ham)

frank_hcv <- frank |>
  left_join(hcv_frank, join_by("nad_addr_to_merge" == "addr")) |>
  filter(!is.na(n_violations))

ham_hcv <- ham |>
  left_join(hcv_ham, join_by("nad_addr_to_merge" == "addr")) |>
  filter(!is.na(n_violations))
```
...
rates <- data.frame(
  county = c(rep("Hamilton County", 5), rep("Franklin County", 5)),
  method = c(rep(c("address matching", "parcel-based geomatching to centroid", "range-based geomatching to centroid", "parcel-based geomatching to polygon", "range-based geomatching to polygon"), 2)),

  match_rate = c(
    sum(!is.na(ham_hcv$addr_matched_parcel_list)) / nrow(ham_hcv),
    sum(!is.na(ham_hcv$nad_closest_parcel_id)) / nrow(ham_hcv),
    sum(!is.na(ham_hcv$degauss_closest_parcel_id)) / nrow(ham_hcv),
    sum(sapply(ham_hcv$nad_intersects_parcel_id, function(x) length(x) > 0 && any(!is.na(x)))) / nrow(ham_hcv),
    sum(sapply(ham_hcv$degauss_intersects_parcel_id, function(x) length(x) > 0 && any(!is.na(x)))) / nrow(ham_hcv),
    sum(!is.na(frank_hcv$addr_matched_parcel_list)) / nrow(frank_hcv),
    sum(!is.na(frank_hcv$nad_closest_parcel_id)) / nrow(frank_hcv),
    sum(!is.na(frank_hcv$degauss_closest_parcel_id)) / nrow(frank_hcv),
    sum(sapply(frank_hcv$nad_intersects_parcel_id, function(x) length(x) > 0 && any(!is.na(x)))) / nrow(frank_hcv),
    sum(sapply(frank_hcv$degauss_intersects_parcel_id, function(x) length(x) > 0 && any(!is.na(x)))) / nrow(frank_hcv)
  ),

  parcel_id_agree_with_addr = c(
    1,
    sum(ham_hcv$nad_parcel_id_agree) / nrow(ham_hcv),
    sum(ham_hcv$degauss_parcel_id_agree) / nrow(ham_hcv),
    sum(ham_hcv$nad_intersects_parcel_id_agree) / nrow(ham_hcv),
    sum(ham_hcv$degauss_intersects_parcel_id_agree) / nrow(ham_hcv),
    1,
    sum(frank_hcv$nad_parcel_id_agree) / nrow(frank_hcv),
    sum(frank_hcv$degauss_parcel_id_agree) / nrow(frank_hcv),
    sum(frank_hcv$nad_intersects_parcel_id_agree) / nrow(frank_hcv),
    sum(frank_hcv$degauss_intersects_parcel_id_agree) / nrow(frank_hcv)
  ) #,

  # land_use_and_value20_agree_with_addr = c(
  #   1,
  #   sum(ham_hcv$nad_land_use_value20_agree) / nrow(ham_hcv),
  #   sum(ham_hcv$degauss_land_use_value20_agree) / nrow(ham_hcv),
  #   sum(ham_hcv$nad_intersects_land_use_value20_agree) / nrow(ham_hcv),
  #   sum(ham_hcv$degauss_intersects_land_use_value20_agree) / nrow(ham_hcv),
  #   1,
  #   sum(frank_hcv$nad_land_use_value20_agree) / nrow(frank_hcv),
  #   sum(frank_hcv$degauss_land_use_value20_agree) / nrow(frank_hcv),
  #   sum(frank_hcv$nad_intersects_land_use_value20_agree) / nrow(frank_hcv),
  #   sum(frank_hcv$degauss_intersects_land_use_value20_agree) / nrow(frank_hcv)
  # )
)
...
summarize_by_quartile <- function(df, name) {
  df |>
    ...
    knitr::kable(digits = 3, caption = paste("Mean Metrics by Deprivation Quartile,", name))
}
...
summarize_by_addr_quartile <- function(df, name) {
  df |>
    ...
    summarise(across(all_of(cols_to_summarize), mean, na.rm = TRUE)) |>
    knitr::kable(digits = 3, caption = paste("Mean Metrics by Address Density Quartile,", name))
}
...
ham_dep_summary |>  # Optional: make quartile labels prettier
  ...
  cols_label(
    dep_quartile = "Tract-level Deprivation Index Quartile"
